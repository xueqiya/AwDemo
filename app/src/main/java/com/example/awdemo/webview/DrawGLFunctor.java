...
class DrawGLFunctor implements AwContents.NativeDrawGLFunctor {
    ...
    public DrawGLFunctor(long viewContext, WebView webView) {
    ...
    public void detach(View containerView) {
        ...
        webView.detachDrawGlFunctor(containerView, mNativeDrawGLFunctor);
    }
    ...
    public boolean requestDrawGL(Canvas canvas, Runnable releasedCallback) {
        ...
        if (sSupportFunctorReleasedCallback) {
            ...
            webView.callDrawGlFunction(canvas, mNativeDrawGLFunctor, releasedCallback);
        } else {
            assert releasedCallback == null;
            webView.callDrawGlFunction(canvas, mNativeDrawGLFunctor);
        }
        return true;
    }
    ...
    public boolean requestInvokeGL(View containerView, boolean waitForCompletion) {
        ...
        if (!sSupportFunctorReleasedCallback
                && !webView.canInvokeDrawGlFunctor(containerView)) {
            return false;
        }

        webView.invokeDrawGlFunctor(
